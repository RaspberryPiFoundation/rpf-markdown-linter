...
import { findLegacyBlocks } from "./legacyBlocks";
import { type LegacyBlockMatch } from "./types";
...
const FIX_ALL_KIND =
  vscode.CodeActionKind.SourceFixAll.append("rpfMarkdownLinter");
...
class LegacyMarkdownQuickFixProvider implements vscode.CodeActionProvider {
  constructor(
    private readonly blocksByDocument: Map<
      string,
      Map<string, LegacyBlockMatch>
    >,
  ) {}
  ...
  public provideCodeActions(
    ...
    for (const diagnostic of context.diagnostics) {
      ...
      const blockId =
        typeof diagnosticCode === "string" ? diagnosticCode : undefined;
      if (!blockId) {
        continue;
      }
    ...
    const fixAllEdit = buildEditFromBlockIds(
      document,
      blockMap,
  ...
  const fixAllInWorkspaceCommand = vscode.commands.registerCommand(
    ...
    async () => {
      ...
      for (const fileUri of markdownFiles) {
        ...
        changedFiles++;
        appliedFixes += matches.filter((match) =>
          Boolean(match.replacement),
        ).length;
        updateDiagnostics(document, collection, blocksByDocument);
      }
